...
public:
    Game_Page(QMainWindow *parent = nullptr);
    ~Game_Page();
    ...
    //深红色方块初始化
    void dred_blocktoscene();
    //“激活”深红色方块
    void dred_blockactivate();
    ...
    //“碰撞检测”
    void collisionDetection_enemy_plane();
    QTimer *enemy_plane_timer;
    void createenemy_plane();// 创造敌机函数
    // 初始化敌机
    enemy_plane* m_selectedPlenemy_plane = nullptr;
    // 已种植的敌机列表
    QList<enemy_plane*> m_plenemy_plane;

    //云朵对象
    cloud m_cloud;
    QTimer *cloud_timer;
    void createcloud();// 创造敌方云朵函数
    // 计时器，用于控制云朵出现和消失时间
    int cloud_counter=-1;
